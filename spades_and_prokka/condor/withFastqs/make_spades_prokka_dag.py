...
def get_args():    
    ...
    parser.add_argument("genus", help="genus")
    parser.add_argument("species", help="species")
    parser.add_argument("gram",help="+ or -")
...
with open(args.input, 'r') as infile:
    ...
    with open(outfileName, "w") as outfile:
        for line in infile:
            ...
            variableMap['genus'] = args.genus
            variableMap['species'] = args.species
            variableMap['gram'] = args.gram
            with open("{0}_spadesProkka.dag".format(variableMap['run']), "w") as dagfile:
                out = template.substitute(variableMap)
                dagfile.write(out)
            outfile.write("SPLICE {0} {0}_spadesProkka.dag\n".format(variableMap['run']))
